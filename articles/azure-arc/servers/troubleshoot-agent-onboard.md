...
|Proxy settings mismatch|`Request error: **Proxy settings mismatch**, Details: server proxy: , bypass: ; client proxy: http://proxy, bypass:   Error="error connecting machine to Azure: server proxy: , bypass: ; client proxy: http://proxy, bypass:` | The azcmagent process is configureed to use a different proxy server than the HIMDS service process. | This is usually due to setting the proxy via environment variable rather than config. Set the proxy explicitly with `azcmagent config set proxy.url`|
